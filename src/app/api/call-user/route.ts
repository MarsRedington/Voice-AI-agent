...
console.log("[DEBUG]", { phone, email })
console.log("[DEBUG]", fakeCallId)
...
await new Promise(resolve => setTimeout(resolve, 1000))
  ...
  const structuredData = {
    ...
    await db.collection("callbacks").doc(fakeCallId).set({
      email,
      phone,
      status: "initiated",
      createdAt: new Date().toISOString(),
    });
console.log("[DEBUG] structuredData:", structuredData)
        await db.collection("callbacks").doc(fakeCallId).update({
      structuredData,
  ...
  body: JSON.stringify({
    ...
    return NextResponse.json({ success: true, fakeCallId }, { status: 200 });
  // } catch (err: unknown) {
